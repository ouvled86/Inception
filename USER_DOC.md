...
- **Main Website**: A WordPress blog accessible at `https://<login>.42.fr`.
- **Admin Panel**: Manage WordPress posts at `https://<login>.42.fr/wp-admin`.
- **Database GUI**: Manage MariaDB tables via Adminer at `https://<login>.42.fr/adminer`.
- **Static Portfolio**: A simple showcase site at `https://<login>.42.fr/static`.
...
Your credentials are generated into `srcs/.env` (from `srcs/.env.example`) when you run `make`.
...
1. Run `docker ps` - all containers should be `Up`.
